...
"""
Contains string parsing functions for MARC field data.
"""
...
def extract_relator_terms(val, from_sf4=False):
    """
    Extract, parse, and compile a list of MARC relator terms or codes,
    such as from a 100$e or 111$j, or a $4, from the given string
    value.

    Pass in the subfield tag (`sftag`) and raw subfield value
    (`sfval`). Returns a list of individual terms, processed as
    follows:
        - Translates MARC relator codes in $4 to the appropriate label.
          (If $4 contains anything other than a MARC relator code, it
          returns an empty list.)
        - Strips WEMI terms in parentheses.
        - Strips punctuation.
        - Parses out individual terms if a non-$4 value contains terms
          separated by ', '.
    """
    if from_sf4:
        term = settings.MARCDATA.RELATOR_CODES.get(val, None)
        return [term] if term else []
    return [strip_wemi(v) for v in strip_ends(val).split(', ') if v]
...
def parse_name_string(name_string):
    ...
    into a data structure such as what fieldparsers 
    `PersonalNameParser` or `OrgEventNameParser` would produce.

    This mainly used to parse author names from Linking Fields
    ...
    def _forename_is_ptitle(forename):
        parsed = person_title(forename)
        return bool(parsed['full_title'])

    is_person = False
    protected = strip_ends(protect_periods(name_string), True, 'right')
    ...
    heading = restore_periods(protected)
    ...
    if is_person:
        ...
        if ptitles:
            ptitles = [restore_periods(pt) for pt in ptitles.split(', ')]
        ...
        for key in ('forename', 'surname', 'fuller_form_of_name'):
            if ret_val[key]:
                ret_val[key] = restore_periods(ret_val[key])
        return ret_val
    ...
    for part_str in protected.split('. '):
        if part_str:
            ...
            if qual_match:
                ...
                part = {'name': restore_periods(name),
                        'qualifier': restore_periods(qualifier)}
            else:
                part = {'name': restore_periods(part_str)}
